...
// Dùng BLE (react-native-ble-plx) để kết nối RFID reader
...
class BLEService {
    ...
    try {
      ...
      if (BLE_CONFIG.MAC_ADDRESS) {
        try {
          ...
          for (const d of knownDevices) {
            if (!seen.has(d.id)) {
              seen.add(d.id);
              onTimThay({ id: d.id, name: d.localName || d.name || 'RFID Reader (saved)', rssi: d.rssi });
            }
          }
        } catch {}
      }

      // Tìm device đã connected
      try {
        const connected = await manager.connectedDevices([BLE_CONFIG.SERVICE_UUID]);
        for (const d of connected) {
          if (!seen.has(d.id)) {
            seen.add(d.id);
            onTimThay({ id: d.id, name: (d.localName || d.name || 'Device') + ' (connected)', rssi: d.rssi });
          }
        }
      } catch {}
      ...
      await new Promise<void>((resolve) => {
        ...
        manager.startDeviceScan(null, { allowDuplicates: false }, (error, dev) => {
          if (error) { clearTimeout(timeout); onLoi(new Error(error.message)); resolve(); return; }
          if (dev && dev.id && !seen.has(dev.id)) {
            const name = dev.localName || dev.name || '';
            if (name) {
              seen.add(dev.id);
              onTimThay({ id: dev.id, name, rssi: dev.rssi });
            }
          }
        });
      });
  ...
  async ketNoi(deviceInfo: any, onTagDoc: (tag: RFIDTag) => void): Promise<void> {
    const deviceId = deviceInfo.id;
    if (this.isConnecting) return;
    this.isConnecting = true;
    this.onTagCallback = onTagDoc;
    ...
    try {
      ...
      for (const svc of services) {
        ...
        for (const ch of chars) {
          console.log('[BLE] Svc:', svc.uuid, 'Char:', ch.uuid,
            'N:', ch.isNotifiable, 'W:', ch.isWritableWithResponse, 'Wn:', ch.isWritableWithoutResponse);
        }
      }

      // Subscribe to all notifiable chars in FFE0 service
  ...
  private handleBLEData(base64Data: string, onTagDoc: (tag: RFIDTag) => void) {
    if (!onTagDoc) return;
    const bytes = [...Buffer.from(base64Data, 'base64')];
    
    // TEMPORARY DEBUG LOGGING to see raw bytes
    console.log('[BLE-RAW]', bytes.map(b => b.toString(16).padStart(2, '0').toUpperCase()).join(' '));
  ...
  private parsePackets(onTagDoc: (tag: RFIDTag) => void) {
    ...
    while (this.rawBuffer.length >= 5 && safety++ < 100) {
      ...
      if (protocol === 'CF') {
        ...
        if (cmd === 0x01 && len >= 6) { 
           // Format CF: byte 6,7 la RSSI (-39.0 dBm -> FE 7A -> -390)
           ...
           if (epcLen > 0 && 11 + epcLen <= totalLen - 2) {
             ...
             console.log('[BLE] ✅ TAG (CF):', epc, 'RSSI:', rssi);
             onTagDoc({ epc, rssi });
           }
        }
        ...
        if (type === 0x02 && (cmd === 0x22 || cmd === 0x27) && payloadLen >= 5) {
          ...
          if (epcLen > 0 && epcLen <= 62) {
            ...
            const epc = epcBytes.map(b => b.toString(16).padStart(2, '0').toUpperCase()).join(' ');
            console.log('[BLE] ✅ TAG (BB):', epc, 'RSSI:', rssi);
            onTagDoc({ epc, rssi });
          }
        }
        this.rawBuffer = this.rawBuffer.slice(totalLen);
  ...
  async batDauQuet(): Promise<void> {
    ...
    console.log('[BLE] Sending START_INVENTORY');
    ...
    if (targetService) {
      ...
      for (const ch of chars) {
        if (ch.isWritableWithoutResponse || ch.isWritableWithResponse) {
          try {
            ...
            console.log('[BLE] ✅ START sent to', ch.uuid);
          } catch (e: any) {
            console.log('[BLE] Write to', ch.uuid, 'failed:', e.message);
          }
        }
      }
    } else {
      try {
        ...
        console.log('[BLE] ✅ START sent (fallback)');
      } catch {
        try {
      ...
      for (const ch of chars) {
        if (ch.isWritableWithoutResponse || ch.isWritableWithResponse) {
          try {
            ...
            console.log('[BLE] ✅ STOP sent to', ch.uuid);
          } catch {}
        }
      }
    } else {
