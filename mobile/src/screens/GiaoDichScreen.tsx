...
export default function GiaoDichScreen() {
  ...
  const { tags, xoaTatCa, luuVaoBo } = useInventoryStore();

  const fetchOrders = async () => {
    setLoading(true);
    try {
      const data = await SyncService.pullOrders();
      // Chỉ lấy phiếu PENDING và IN_PROGRESS
      setOrders(data.filter((o: Order) => o.status === 'PENDING' || o.status === 'IN_PROGRESS'));
    } catch (e: any) {
      Alert.alert('Lỗi', 'Không thể lấy danh sách Phiếu từ máy chủ.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Giao diện chọn đơn hàng
  if (!selectedOrder) {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <Text style={styles.headerTitle}>Phiếu Giao Dịch</Text>
          <TouchableOpacity onPress={fetchOrders} style={styles.btnSync}>
              ...
              >
                ...
                <View style={styles.orderStats}>
                  <Text style={styles.statText}>
                    📦 {item.items.reduce((acc, curr) => acc + curr.quantity, 0)} SP yêu cầu
                  </Text>
                  <Text style={styles.statText}>
                    ✅ {item.items.reduce((acc, curr) => acc + curr.scannedQuantity, 0)} SP đã quét
                  </Text>
                </View>
              </TouchableOpacity>
            )}
            ListEmptyComponent={
              <Text style={styles.emptyText}>Chưa có phiếu Nhập/Xuất nào cần xử lý.</Text>
            }
          />
        )}
      </View>
  ...
  // Chế độ Pick Hàng (Đã chọn đơn)
  
  // Tính toán đối chiếu
  const tagList = Object.values(tags).filter(t => t.coMat);
  const validProductNames = selectedOrder.items.map(i => i.product.name);
  // Chỉ lấy những thẻ có tên sản phẩm khớp với đơn hàng
  const validTagList = tagList.filter(t => validProductNames.includes(t.tenHienThi));
  
  const totalScanned = validTagList.length;
  ...
  selectedOrder.items.forEach(item => {
    sessionScannedCounts[item.id] = validTagList.filter(t => t.tenHienThi === item.product.name).length;
  });
  ...
  const submitSession = async () => {
    ...
    try {
      await SyncService.pushSession({
        ...
        orderId: selectedOrder.id, // Backend sẽ dùng cái này
        scans: validTagList.map(t => ({ epc: t.epc, rssi: t.rssi, time: new Date(t.lanQuetCuoi) }))
      } as any);
      
      Alert.alert('Thành công', 'Đã lưu kết quả giao dịch về Server.', [
        { text: 'OK', onPress: () => setSelectedOrder(null) }
      ]);
    } catch (e) {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity 
          style={[styles.btnScan, dangQuetInventory && styles.btnScanActive]} 
          onPress={startScan}
        >
          <Text style={styles.textBtnScan}>{dangQuetInventory ? '🛑 Dừng Quét' : '▶️ Bắt đầu Quét'}</Text>
        </TouchableOpacity>
        
        <TouchableOpacity 
          style={[styles.btnSave, isSaving && { opacity: 0.5 }]} 
...
const styles = StyleSheet.create({
  ...
  headerActive: {
    ...
    backgroundColor: '#1a2a3e', flexDirection: 'row', alignItems: 'center', gap: 12
  },
  ...
  checklistItem: {
    ...
    borderLeftWidth: 4, borderLeftColor: '#4dd0e1'
  },
