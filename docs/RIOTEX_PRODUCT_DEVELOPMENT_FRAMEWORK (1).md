...
| `WAREHOUSE_STAFF` | Tạo transfer nghiệp vụ kho, scan verify, kiểm kê, confirm transfer, theo dõi tồn kho và live view | Web, Mobile |
...
1. `SUPER_ADMIN` hoặc `WAREHOUSE_STAFF` tạo transfer ở trạng thái `PENDING`.
...
1. `SUPER_ADMIN` hoặc `WAREHOUSE_STAFF` tạo transfer từ `WAREHOUSE` đến location khách hàng.
...
| Transfer create | Yes | No | Yes |
| Transfer cancel | Yes | No | No |
...
- `SUPER_ADMIN` và `WAREHOUSE_STAFF` đều được tạo transfer.
- Chỉ `SUPER_ADMIN` được hủy transfer.
...
| Main Flow | 1. `SUPER_ADMIN` hoặc `WAREHOUSE_STAFF` tạo transfer `PENDING`.<br>2. `WAREHOUSE_STAFF` scan verify từng asset.<br>3. Hệ thống ghi `scannedAt` cho transfer items.<br>4. `WAREHOUSE_STAFF` confirm transfer.<br>5. Hệ thống cập nhật location và status. |
...
| Main Flow | 1. `SUPER_ADMIN` hoặc `WAREHOUSE_STAFF` tạo transfer `WAREHOUSE_TO_CUSTOMER`.<br>2. Hệ thống tự hoàn tất transfer.<br>3. Asset được cập nhật `OUT_OF_STOCK` và location đích là customer. |
...
| FR-007 | Transfer creation and cancellation | `SUPER_ADMIN`, `WAREHOUSE_STAFF` for create; `SUPER_ADMIN` for cancel | Tạo/hủy transfer đúng business rules |
...
| Transfers | POST | `/api/transfers` | `SUPER_ADMIN`, `WAREHOUSE_STAFF` | Create transfer |
