...
export function BulkEditModal({ selectedEpcs, onClose }: BulkEditProps) {
  ...
  const { data: productsData, isLoading: isLoadingProducts } = useQuery({
    ...
    queryFn: () => api.getProducts(`limit=20${search ? `&search=${search}` : ''}`),
  });
  ...
  const bulkUpdateMutation = useMutation({
    mutationFn: async () => {
      // If a product is selected, assign it to all selected tags
      if (selectedProductId) {
        await api.assignTags({ productId: selectedProductId, tagIds: selectedEpcs });
      }
      
      // If location is provided, update it for all selected tags
      if (location.trim()) {
        await Promise.all(
          selectedEpcs.map(epc => api.updateTag(epc, { location: location.trim() }))
        );
      }
    },
    onSuccess: () => {
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
        ...
        <div className="p-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
          ...
          <button 
            ...
            disabled={bulkUpdateMutation.isPending || (!selectedProductId && !location)}
