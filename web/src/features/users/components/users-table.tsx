...
export function UsersTable({
  ...
  const renderSortableHeader = (label: string, sortKey: string, align: 'left' | 'center' | 'right' = 'left', customWidth?: string) => {
    ...
    return (
      <th
        className={`px-5 py-4 font-black text-[11px] text-slate-500 tracking-widest ${isSortable ? 'cursor-pointer hover:bg-slate-50 transition-colors group' : ''} ${customWidth || ''}`}
        ...
        <table className="w-full text-sm text-left">
          <thead className="sticky top-0 z-10 bg-white shadow-sm border-b border-slate-100">
            <tr>
              ...
              <th className="px-5 py-4 font-black text-[11px] text-slate-500 tracking-widest text-right">Thao tác</th>
            </tr>
          </thead>
            ...
            {users.map((u, i) => {
              ...
              return (
                <tr key={u.id} className={`hover:bg-slate-50/50 transition-colors group ${selectedIds.includes(u.id) ? 'bg-[#04147B]/5' : ''}`}>
                  ...
                  <td className="px-5 py-4 text-left">
                    <span className={`px-3 py-1.5 text-[9px] font-black tracking-wider rounded-full ${getRoleBadge(roleDisplay)} ${isDisabled ? 'opacity-50 grayscale' : ''}`}>
                      {roleDisplay}
                    </span>
                  </td>
                  <td className="px-5 py-4 text-left">
