...
class BatchFlowTest {
  ...
  static getTags(token, searchPrefix) {
    return new Promise((resolve, reject) => {
      const options = {
        hostname: 'localhost',
        port: 3000,
  ...
  static assignProduct(token, tagIds, productId) {
    return new Promise((resolve, reject) => {
      const data = JSON.stringify({
        tagIds,
        productId
  ...
  static async run() {
    ...
    try {
      ...
      const token = await BatchFlowTest.login();
      console.log('Login OK\n');

      // Step 2: Create test product
      console.log('Step 2: Create test product...');
      const productId = await BatchFlowTest.createProduct(token);
      if (!productId) {
      ...
      const tagsResult = await BatchFlowTest.getTags(token, 'TEST');
      ...
      if (tagsResult.data?.length > 0 && productId) {
        ...
        const assignResult = await BatchFlowTest.assignProduct(token, tagIds, productId);
        console.log('Assign result:', JSON.stringify(assignResult, null, 2));
      }

      console.log('\n=== TEST COMPLETE ===');
    } catch (error) {
      BatchFlowTest.handleError(error);
    }
  }
