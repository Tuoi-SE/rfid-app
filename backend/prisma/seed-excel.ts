      ...
      for (const row of data) {
        ...
        if (isCol0Number) {
          ...
          if (!sku || sku === 'NO_SKU' || sku === 'Cái' || sku === 'Bộ') {
            sku = ExcelSeedRunner.generateSku();
          }
          ...
          try {
            ...
            console.error(`\nLỗi khi import sản phẩm: ${name} (SKU: ${sku})`, error);
          }
        } else {
...
ExcelSeedRunner.run()
  .catch(ExcelSeedRunner.handleError)
  .finally(ExcelSeedRunner.disconnect);
